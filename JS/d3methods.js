const Background=function(){
    ...
    obj.initalize=function(){
        ...
        if(isFirefox){obj.numberOfStars=50;
    ...
    obj.addInitialStars=function(){
        ...
        {this.stars[i]={width:this.getRandomWidth(),
            height:this.getRandomHeight(),speed:this.setSpeed(),
        };}
    d3.select('body')
        .selectAll('.stars')
        .data(function(){return this.stars;}
        .bind(this)).enter()
        .insert('div').attr('class','stars').style('left',function(star){
    ...
    obj.moveStars=function(){
        ...
        return `${data.height}px`;}).style('width',function(data){return `${data.speed}px`;}).style('height',function(data){return `${data.speed}px`;})
            .attr('class',function(data){if(data.height<0){return 'stars remove';} return 'stars';});

        d3.selectAll('.remove')
            .remove();for(let i=0;i<this.stars.length;i++){
    ...
    obj.addStars=function(){
        ...
        d3.select('body')
            .selectAll('.stars')
            .data(this.stars).enter()
            .append('div')
            .attr('class','stars').style('left',function(){return `${newStar.width}px`;}).style('top',
            function(){
